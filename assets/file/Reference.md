...
#! => 告诉系统脚本需要什么解释器来执行，通常有 #!/bin/ash 和 #!/bin/sh
./x.sh 不能写成 x.sh => 后者代表到 PATH 中寻找 x.sh 文件并执行
可以直接作为解释器的参数运行SHELL脚本:  `/bin/sh sample.sh`;
`#` 为单行注释标记；没有多行注释，可以用函数`{}`代替；一行可执行多条命令，使用引号分隔；
定义变量时不用`$`符号,等号和变量之间不能有空格；
使用变量时要使用`$`符号,同时可以加花括号以识别边界:  `echo $name`, `echo ${name}`;
`readonly`关键字定义只读变量；`unset`关键字删除变量(只读除外);
运行SHELL脚本时存在三种变量: 局部变量、环境变量、SHELL变量;
SHELL中的特殊变量: 
      $0    当前脚本的文件名
